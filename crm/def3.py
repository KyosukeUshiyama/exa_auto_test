...
def getCustomerInfo(start_row, start_column, sheet):
    customer_info = {
        ...
        "s_b": sheet.cell(row = start_row + 3, column = start_column).value, \
        ...
        "card_kana": sheet.cell(row = start_row + 11, column = start_column).value, \
...
def  convertCustomerInfo(customer_info):
    street_building = customer_info["s_b"]
    ...
    converted = {
        "lastkana": customer_info["kana"].split()[0], \
        "firstkana": customer_info["kana"].split()[1], \
        "lastname": customer_info["name"].split()[0], \
        "firstname": customer_info["name"].split()[1], \
        "zipcode1": customer_info["zipcode"].split("-")[0], \
        "zipcode2": customer_info["zipcode"].split("-")[1],\
        "street": street, \
        "building": building, \
        "telno": customer_info["telno"], \
        "sex": customer_info["sex"], \
        "birth": customer_info["birth"], \
        "mail": customer_info["mail"], \
        "imei": customer_info["imei"], \
        "card_no": customer_info["card_no"], \
        "card_expire": customer_info["card_expire"], \
        "card_lastkana": customer_info["card_kana"].split()[0], \
        "card_firstkana": customer_info["card_kana"].split()[1], \
        "card_birth": customer_info["card_birth"], \
        "security_cd": customer_info["security_cd"]
    }
    return converted
